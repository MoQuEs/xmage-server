...
java -Xms$Xms -Xmx$Xmx -XX:MaxPermSize=$MaxPermSize -adminPassword=$adminPassword -Djava.security.policy=./config/security.policy -Djava.util.logging.config.file=./config/logging.config -Dlog4j.configuration=file:./config/log4j.properties -jar ./lib/$LIB